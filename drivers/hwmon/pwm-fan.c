...
static ssize_t set_cool(struct device *dev, struct device_attribute *attr,
		const char *buf, size_t count)
{
	struct pwm_fan_ctx *ctx = dev_get_drvdata(dev);
...
static ssize_t show_cool(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	struct pwm_fan_ctx *ctx = dev_get_drvdata(dev);
	int lenght = 0, i;
...
static SENSOR_DEVICE_ATTR(fan_speed, S_IRUGO | S_IWUSR, show_cool, set_cool, 0);
