	...
	struct vframe_s *vf, int outfmt)
{
	...
	printk("tobetter -- %s,%d -- depth=%d, fmt=%08x, cpu=%02x\n",
			__func__, __LINE__, vf->bitdepth, intfmt, get_cpu_type());
#if 0
	...
	printk("tobetter -- %s,%d -- %dx%d@%dx%d => %dx%d@%dx%d\n",
			__func__, __LINE__,
			input_width, input_height, input_x, input_y,
			ge2d_config.dst_para.width,
			ge2d_config.dst_para.height,
			ge2d_config.dst_para.left,
