...
static int meson_cpufreq_init(struct cpufreq_policy *policy)
{
	...
	if (board_is_odroidn2() || board_is_odroidc4()
      || board_is_odroidhc4() || board_is_bananapi_m5()) {
		int i = 0;
...
{
	if (board_is_odroidc4() || board_is_odroidhc4() || board_is_bananapi_m5())
		return get_max_freq_cortex(0, str);
	return -EINVAL;
}
...
static int meson_cpufreq_probe(struct platform_device *pdev)
{
	...
	} else if (board_is_odroidc4() || board_is_odroidhc4() || board_is_bananapi_m5()) {
